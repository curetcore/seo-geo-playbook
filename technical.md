...
> Skill del agente `seo-marketing`. Cubre Core Web Vitals, crawlability, page speed, seguridad, y arquitectura del sitio.
...
| Metrica | Bueno | Necesita mejora | Malo | Herramienta |
|---------|-------|-----------------|------|-------------|
...
> **IMPORTANTE**: INP reemplazo a FID en marzo 2024. NUNCA referenciar FID.
...
// Contenido critico se renderiza en el servidor = LCP rapido
...
- [ ] Preconnect a origenes de terceros criticos
- [ ] Imagenes en WebP/AVIF (next/image lo hace automatico)

---

## Optimizacion INP

INP mide la latencia de la interaccion mas lenta del usuario. Aplica a clicks, taps, y keyboard input.
...
const HeavyChart = dynamic(() => import("@/components/chart"), {
  loading: () => <div className="h-64 animate-pulse bg-base-200 rounded-lg" />,
...
- [ ] Listas largas usan virtualizacion (react-window / tanstack-virtual)
...
### 1. Dimensiones explicitas en imagenes y videos
...
function ProductCardSkeleton() {
  return (
    <div className="w-full h-80 animate-pulse rounded-lg bg-base-200" />
  );
}
```
...
- [ ] Todas las imagenes/videos tienen width y height explicitos
- [ ] Fuentes usan `display: swap` con `size-adjust`
- [ ] Ads/embeds tienen contenedores con aspect ratio fijo
- [ ] Sin insercion dinamica de contenido above-the-fold
...
export default function robots(): MetadataRoute.Robots {
  ...
  return {
    rules: [
      ...
      // Bots de IA que generan trafico (permitir)
      {
...
export default function Dashboard() {
  return (
    <main>
      ...
      <Suspense fallback={<div className="h-64 animate-pulse bg-base-200 rounded-lg" />}>
        <SlowComponent />
      </Suspense>
    </main>
  );
